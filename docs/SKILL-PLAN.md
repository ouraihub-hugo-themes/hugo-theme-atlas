...
## 19. 第三轮端到端：补完之后再验（2026-09-04）

第 18 节补完 12 条，那份 skill 还没有再跑过。这一轮验它，并且放行拷贝命令 ——
上一轮沙箱挡了 `cp`，agent 改用 `themesDir="."` 绕过，而「把主题拷进 themes/」
正是 clone-and-own 分发的第一步，等于那一步一次都没验过。

### 19.1 产出

主题真的拷进了 `themes/hugo-theme-atlas/`（375 个文件），16 页内容、39 页产物、
零警告。上一轮绕过的那一步这次过了。

### 19.2 报告的 8 条，逐条核过

7 真 1 半真。这一轮的报告比上一轮准得多 —— 每条都带文件与行号。

| 它说的 | 核的结果 |
|---|---|
| `steps` 必须用 `{{% %}}`，文档写的是 `{{< >}}` | **真，且是生成器的系统性错误** |
| `include` 的 `code` 是布尔，占位符诱导写成语言名 | 真，`include.html:32` |
| `xref` 忽略自定义 `id`，产出死链 | **真，而且是主题 bug 不是文档缺口** |
| `featured_image` 是治理模式枚举不是图片路径 | 真，`featured.html:27` |
| SKILL.md 的图标数 76 是错的 | 真，72 个（四个 `$comment*` 键） |
| 类名前缀没有文档，验证时容易假阴性 | 真，但这条我按「不写手写映射表」处理 |
| `{.steps}` 的位置没给示例 | 真 |
| `data.md` 的渠道字段漏了 `title` | 真，白名单有两支我只抽了一支 |

半真的那条是类名映射：症状真（它 grep `td-fig` 落空），但补一张手写的
「组件 → 类名」表会立刻成为第二处会漂的地方。生成器里没有可靠判据能抽出
「这个组件的顶层类名是什么」—— 类名散在模板各处的 HTML 里。所以这条不补表，
留给下一轮看它是否真的再次绊人。

### 19.3 `steps` 那条是这一轮最贵的发现

生成器对 30 个 shortcode 一律印 `{{< >}}`。而 `steps` 的正文是 raw `.Inner`
（前后留空行给 Goldmark 看块边界），用 `{{< >}}` 调用时正文不过 Markdown ——
`### 标题` 变成页面上的字面量，零警告、exit 0。

实测确认了边界：`card` 正文用 `{{< >}}` 出 `<strong>`（它自己调 render-block），
只有 `steps` 有这个问题。判据写成通用形状而不是写死 `td-steps`：`{{ .Inner }}`
独占一行、前后空行、且全篇不调 render-block。第二个这样的 shortcode 出现会被
自动认出来。

exampleSite 里 `{{% steps %}}` 是整个示例站唯一的 `%` 调用 —— 所以「照抄示例」
也救不了，因为作者不会知道那一处的 `%` 是必需的而不是风格。

### 19.4 `xref` 死链：改主题，不改文档

`{{< fig num="1" id="fig-shell" >}}` 加 `{{< xref fig="1" />}}` 产出
`href="#fig-1"` 对 `id="fig-shell"`。实测复现，零警告，而链接文字是对的，
所以页面上看不出来。

两条修法。**查登记表不成立**：`tdBookTargetNums` 里有真 id，但 xref 是行内的，
页面里靠后的目标此刻还没登记（`book-figures` 那类列表能查表，是因为它整页渲染
完才读，跨页还先强取 `.Content`）。前向引用是常见写法，不能只对后向引用正确。

**所以警告放在作者写下自定义 id 的那一处** —— `target-head.html`，不依赖渲染
顺序。消息直接给出改法（`xref anchor="fig-shell"`）。id 本身照用：稳定深链是
合法需求，只是与编号引用不能混着来。

`tests/invalid/book.md` 加了负例。原有的 `fig-dup`/`fig-other` 两个用例也带
自定义 id，所以 expect 一并跟上。故意改错一条验过断言真在起作用。

### 19.5 我自己上一轮写错的一处

核 `featured_image` 时发现的：`params.md` 那行「页面级键：`reading_width`、
`featured_image`、`tone`、`note`、`pem`」里，后三个**根本不是页面级键** ——
`tone` 是 `badge` 的参数、`note` 是下载渠道的字段、`pem` 是 filetree 认的扩展名。
那是我上一轮用 `"key" "..."` 一把捞出来的，捞进了三个无关的东西。

改成从消费方抽之后，反而补上了两个我从没写过的真键：`search_boost`（1–5 档位，
不是倍数）与 `search_exclude`（布尔）。手写一张表的代价在这里很具体：它既会
凭空造出不存在的键，又会漏掉存在的。

### 19.6 SKILL.md 的数字加了门禁

`check-skill-counts.js`，第八个检查器。SKILL.md 保持手写 —— 它的价值是「先读
哪一份」那种判断，不是结构。但里面五个数字是对外承诺，而没有任何东西看着它们。
新检查器抓出图标数 76→72，另外四个证实是对的。

## 20. 类名与 token：两处只有散文没有门禁的地方（2026-09-04）

### 20.1 第三轮第 6 条缺口，我上一轮定性错了

症状是 agent 想给 fig 加样式，grep `td-fig` 落空 —— 模板发的是 `td-book-figure`。
我上一轮记成「缺一张组件→类名表」并推迟。**那个定性是错的**：产物里 366 个类，
表给了也读不动，且会立刻成为第二处会漂的地方。

真正缺的是那一跳的方法。实测覆盖率：22 个 landing section 全部、9 个围栏里 7 个
直接带 `class="… td-x …"` 字面量；剩下 15 个 shortcode 分三类 —— 委托 partial
（10 个，路径就写在同一文件里）、标记在父 shortcode（`field`/`tab`）、不发壳
（`param`/`include`/`comment`）。

两件 grep 问不出来的事也写进去了：变体类名是 `printf "td-callout--%s"` 渲染时
拼的，`td-callout--note` 在任何模板里都搜不到；以及**「某个类没有 CSS 规则」在
这个主题里是常态** —— 语义标记、以及基类已承担该状态的变体（`.td-badge` 自己
就是 neutral，所以 `td-badge--neutral` 无规则）。不知道这条的 agent 会把正常
状态当缺陷去"修"。
...
**静态 join** 有四种噪音源 —— id（`td-main`）、`data-td-*`（`\btd-` 会在
`data-td-language` 里命中）、`--td-*`、以及作为 partial 参数传的类名
（`td-callout-marker` 因此被误报成死代码）。而且对拼接类名注定只抽到半截。

**从产物抽变体查 CSS** 干净，但 17 条假阳性起步：种类标记共用基类样式
（`td-book-figure--fig`）、默认值无需覆盖。每加一个种类标记就要加一条豁免，
那种检查器的终局是被跳过。

### 20.3 但手写的断言必须上门禁

这是我上一轮真正的错。拒检查器（对）和给自己写下的九条断言上门禁（该做而没做）
是两件事 —— 项目里其他五份参考页全是生成加门禁，那一节成了唯一无人看管的
事实声明。

判据就用那一节自己给出的那句话：模板里有没有 `class="… td-x …"`。有就该直接
去读，没有就必须出现在三份名单之一里。四种破坏各验一次全红。门禁**第一次跑就
抓到我一处偷懒**：「the four `book-*` lists」——读者还得自己猜是哪四个。

同时删掉三个需要构建才能验的精确数（366/36/40）。带着无人看管的数字比不带更糟。

### 20.4 token 清单：shadcn-svelte 指出的缺口

用户给了 https://www.shadcn-svelte.com/docs 参照。它对「用户怎么知道改什么」
给的答案是三层，**没有一层是类名表**：组件页直接展示完整源码、theming 页把变量名
穷举列出、没有 props 表（从源码类型推）。第一层印证了 20.1 的方向。

但第二层它做了我没做的事。本主题这一层存在（`theme.css`，131 个 token），却
**一处都没穷举过** —— README 只给三个示例。而它比类名更该有清单：token 有限、
命名有规范、改一个值换整套外观，且可生成可门禁。

第六个生成器 `gen-skill-tokens.js`。抽 `theme.css` 五个块：`:root` 原始值、
深色覆盖（30 个）、字体预设覆盖、`@theme inline` 映射（34 个）、`@theme` 设计
常量（8 个）。17 个 `--td-*` 没进映射，按名字分三组：status emphasis、布局量度、
字族角色。

三样东西从消费方抽而不是叙述 —— 读 `-emphasis` 的组件样式表、在媒体查询里被
重声明的量度、`:root` 里的纯别名关系。最后一条让「改 token 值」这种破坏也能被
门禁抓到：`--td-accent: var(--td-link)` 改成字面值就取消了别名，Dark 列跟着变。

### 20.5 最要紧的一条：`--td-*` 还是 `--color-*`

站点作者该设 `--td-*`。`--color-*` 是 `@theme inline` 里的映射，只喂 Tailwind
的 utility 生成；组件 CSS 大量直接读 `--td-*`。设错那层的表现是**一部分变了
一部分没变**，构建照常绿 —— 本主题的典型静默失败。
